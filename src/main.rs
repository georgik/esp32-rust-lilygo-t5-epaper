...
fn main() -> ! {
    ...
    // };
    // let mut nothing_array = alloc::vec![0u8; 25000];
    // println!("{:?}",nothing_array[0]
    let mut buf_array = alloc::vec![0u8; 4050];
    ...
    loop {
        ...
        println!("Updating frame2");
        ssd1680.update_bw_frame(&mut spi, display_bw.buffer()).unwrap();
        println!("Updating display2");
        ssd1680.display_frame(&mut spi, &mut delay).unwrap();

        println!("Updating frame3");
        ssd1680.update_bw_frame(&mut spi, display_bw.buffer()).unwrap();
        println!("Updating display3");
        ssd1680.display_frame(&mut spi, &mut delay).unwrap();
